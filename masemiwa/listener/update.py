...
from masemiwa.listener import E404_HTTP_RETURN_CODE_NO_CONNECTION_TO_SEEK, \
    ...
    E406_HTTP_RETURN_CODE_NOT_ACCEPTABLE

logger = logging.getLogger(__name__)


class HandleUpdate(HandleIO):
    """
    handles all UPDATE logic for a single model link (can have several content_blob)
    """
    __link: str
    ...
    def process(self) -> (str, int):
        ...
        if self.__valid:
            # pass to morre-queue
            if not self._send():
                return "FATAL unable to add {0} to update queue; check the logs!".format(
                    self.__link), E500_HTTP_RETURN_CODE_INTERNAL_ERROR
        else:
            return "model {0} cannot be used with MORRE - nothing to insert; nothing deleted".format(
                self.__link), E406_HTTP_RETURN_CODE_NOT_ACCEPTABLE

        return "added {0} to update-queue".format(self.__link), E200_HTTP_RETURN_CODE_SUCCESS_ADDED
    ...
    def _send(self) -> bool:
        ...
        if len(blobs) > 0:
            morre.the_queue.add_to_update_queue_and_eventually_start(blobs)
            return True
        return False
